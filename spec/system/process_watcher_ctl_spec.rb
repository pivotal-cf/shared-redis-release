...
describe 'process-watcher startup logging' do
  it 'does not log that another processmonitor process is running' do
    ...
    log_output = bosh.ssh(deployment_name, Helpers::Environment::BROKER_INSTANCE_NAME, "sudo cat #{PROCESS_WATCHER_PATH}")
...
def clear_log_and_restart_process_watcher
  ...
  bosh.ssh(deployment_name, Helpers::Environment::BROKER_INSTANCE_NAME, 'sudo /var/vcap/bosh/bin/monit restart process-watcher')
  expect(bosh.wait_for_process_start(deployment_name, Helpers::Environment::BROKER_INSTANCE_NAME, 'process-watcher')).to eq(true)
end

def clear_process_watcher_logs
  bosh.ssh(deployment_name, Helpers::Environment::BROKER_INSTANCE_NAME, "sudo truncate -s 0 #{PROCESS_WATCHER_PATH}")
end

def start_fake_process_monitor
  Thread.new do
    bosh.ssh_with_error(deployment_name, Helpers::Environment::BROKER_INSTANCE_NAME, 'bash -c "exec -a fakeprocessmonitor sleep 10000"')
  end
end

def stop_fake_process_monitor(fake_thread)
  bosh.ssh(deployment_name, Helpers::Environment::BROKER_INSTANCE_NAME, 'sudo kill `pidof fakeprocessmonitor`')
  fake_thread.kill
end
